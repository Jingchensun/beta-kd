...
class MobileLlamaForCausalLM(LlamaForCausalLM, MobileVLMMetaForCausalLM):
    ...
    def forward(
        ...
        if not use_cache:
            ...
            # Handle Flash Attention returning None for attention weights
            if text_mask is None:
                t_v_mask = None
                first_a = None
            elif dec_a[0] is None:
                # Flash attention returns None, use model config to get head count
                num_heads = self.config.num_attention_heads
                t_v_mask = vision_mask.unsqueeze(1).unsqueeze(-1).repeat(1,num_heads,1,vision_mask.shape[-1]) * text_mask.unsqueeze(1).unsqueeze(-2).repeat(1,num_heads,text_mask.shape[-1],1)
                first_a = None
                del vision_mask
                del text_mask
            else:
                # Standard path when attention weights are available
